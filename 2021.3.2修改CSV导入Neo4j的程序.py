#该代码本欲修改“封装类别.py”，但打算直接读取数据库数据并保存到neo4j，故暂时搁置
import csv
...
class keshi(object):
    ...
    def duqu(self):
        ...
        base_route = 'D:\\ruanjiananzhaung\\pycharm_shequban\\pycharmDATA\\2020.7.23neo4j1\\output\\'
        # csv 读取
        csv_file1 = csv.reader(open(
            base_route+'title_authors.csv',
            'r', encoding='utf-8'))
        print(csv_file1)  # 打印出来的csv_file1只是一个对象的模型
        csv_file2 = csv.reader(open(
            base_route+'all_information.csv',
            'r', encoding='utf-8'))
        print(csv_file2)  # 打印出来的csv_file2只是一个对象的模型
        csv_file3 = csv.reader(open(
            base_route+'title_keywords.csv',
            'r', encoding='utf-8'))
        print(csv_file3)  # 打印出来的csv_file3只是一个对象的模型
        return csv_file1, graph, csv_file2, csv_file3
class Create(object):
...
def main():
    y=keshi()
    csv_file1, graph, csv_file2, csv_file3=y.duqu()
    x=Create(csv_file1, graph, csv_file2, csv_file3)
    x.file1()
    x.file2()
    x.file3()
    x.yunxing()
